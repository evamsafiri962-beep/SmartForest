  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
        <MapPin className="w-6 h-6 text-emerald-400" /> Manage Zones: {forest?.name || 'Unknown Forest'}
      </h1>
      ...
      <form onSubmit={addZone} className="bg-gray-900/50 border border-gray-800 rounded-xl p-5 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input type="text" placeholder="Zone Name" value={newZone.name} onChange={(e) => setNewZone({...newZone, name: e.target.value})} className="bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2 text-white" required />
          <input type="text" placeholder="Coordinates" value={newZone.coordinates} onChange={(e) => setNewZone({...newZone, coordinates: e.target.value})} className="bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2 text-white" />
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-800/50 border-b border-gray-800">
            <tr><th className="p-3 text-sm text-gray-400">Name</th><th className="p-3 text-sm text-gray-400">Coordinates</th><th className="p-3 text-sm text-gray-400">Status</th><th className="p-3 text-sm text-gray-400">Actions</th></tr>
          </thead>
          <tbody>
            {zones.map(z => (
              <tr key={z._id} className="border-b border-gray-800">
                ...
                <td className="p-3">
                  <button onClick={() => deleteZone(z._id)} className="text-red-400 hover:text-red-300 transition">
