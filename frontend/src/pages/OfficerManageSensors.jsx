  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
        <Cpu className="w-6 h-6 text-emerald-400" /> Manage Sensors: {forest?.name || 'Unknown Forest'}
      </h1>
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-800/50 border-b border-gray-800">
            <tr><th className="p-3 text-sm text-gray-400">ID</th><th className="p-3 text-sm text-gray-400">Type</th><th className="p-3 text-sm text-gray-400">Zone</th><th className="p-3 text-sm text-gray-400">Status</th><th className="p-3 text-sm text-gray-400">Battery</th><th className="p-3 text-sm text-gray-400">Actions</th></tr>
          </thead>
          <tbody>
            {sensors.map(s => (
              <tr key={s._id} className="border-b border-gray-800">
                ...
                <td className="p-3">
                  <button onClick={() => deleteSensor(s._id)} className="text-red-400 hover:text-red-300 transition">
